...
namespace DocAssistant.Prompt
{
	 public class AdaptiveCardPrompt : Prompt<JObject>
    {
	    private readonly IStateService _serviceState;

	    public AdaptiveCardPrompt(string dialogId, IStateService serviceState , PromptValidator<JObject> validator = null)
        ...
        protected override async Task OnPromptAsync(ITurnContext turnContext, IDictionary<string, object> state, PromptOptions options, bool isRetry, CancellationToken cancellationToken = default)
        {
	        ...
	        {
		        await _serviceState.ConversationState.ClearStateAsync(turnContext);
		        await _serviceState.UserState.ClearStateAsync(turnContext);
	        }
        }
 
        protected override Task<PromptRecognizerResult<JObject>> OnRecognizeAsync(
